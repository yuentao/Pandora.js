...
(function() {
    ...
    if (!getRoot("alertFontSize")) rootText += "/*alert字体大小*/--alertFontSize:.3rem;";
    ...
    window.alert = content => {
        let mask = document.createElement("div"),
            maskBg = getRoot("alertBg") ? getRoot("alertBg") : "rgba(0, 0, 0,.45)",
            div = document.createElement("div"),
            timeout, Theme = getRoot("alertTheme") ? getRoot("alertTheme") : "rgba(0, 0, 0,.45)",
            fontSize = getRoot("alertFontSize") ? getRoot("alertFontSize") : ".3rem",
            color = getRoot("alertColor") ? getRoot("alertColor") : "#fff";
...
const PandoraAPI = class {
    constructor(element = "html") {
        ...
        this.html = content => {
            const ele = this.get;
            if (content) {
                for (let el of ele.children) ele.removeChild(el);
    ...
    return class extends SuperClass {
                    ...
                    getParam = url => {
                        if (url.indexOf("?") > 0) {
                            ...
                            switch (config.storage) {
                                case "session":
                                    sessionStorage.setItem(`Pd-data[${url.split("?")[0] + ""}]`, JSON.stringify(dataObj));
                                    break;
                                case "local":
                                    localStorage.setItem(`Pd-data[${url.split("?")[0] + ""}]`, JSON.stringify(dataObj));
                                    break;
                            }
                            return url.split("?")[0];
                        }
        ...
        Switcher(options) {
                ...
                const touchStart = event => {
                    if (event.target.getAttribute("data-cancel") || event.target.tagName.toUpperCase() == "A") return false;
                    event.preventDefault();
                    ...
                    let {
                        left,
                        top
                    } = parentEle.parentElement.getBoundingClientRect();
                    switch (config.Direction) {
                        case "horizontal":
                            startX = pageX - left;
                            break;
                        ...
                        default:
                            switch (config.Direction) {
                                ...
                                case "vertical":
                                    if (startY > curY) {
                                        if (cur != total - 1)
                                            parentEle.style.transform = `translate3d(0,${-1 * (startY - curY + (childH * cur))}px,0)`;
                                    } else {
                                        if (cur != 0)
                                            parentEle.style.transform = `translate3d(0,${-1 * (childH * cur) + Math.abs(curY - startY)}px,0)`;
                                    }
                                    break;
                            }
                            break;
        ...
        Dialog(options) {
                ...
                const confrimBtn = config.Confrim.btn ? new PandoraAPI(config.Confrim.btn) : null,
                    cancelBtn = config.Cancel.btn ? new PandoraAPI(config.Cancel.btn) : null;

                if (config.Direction !== "none") this.css({
                ...
                const closeDialog = () => {
                    ...
                    config.Confrim.btn && confrimBtn.unbind("click");
                ...
                const openDialog = param => {
                    new Promise(next => {
                        ...
                        const { close } = this;
                        //确认按钮被点击
                        config.Confrim.btn && confrimBtn.bind("click", () => {
                            config.Confrim.callback({ param: param ? param : null, close });
                        });
                        //取消按钮被点击
                        config.Cancel.btn && cancelBtn.bind("click", () => {
        ...
        ImgUpload(options) {
                ...
                if (!config.Events.ready) {
                    if (!document.getElementById("Pd-upload-Mask")) {
                        mask = document.createElement("div");
                        let svg = new Image();
                        svg.src = "//b.pandorajs.com/Pandora/Pandora/src/loader.svg";
